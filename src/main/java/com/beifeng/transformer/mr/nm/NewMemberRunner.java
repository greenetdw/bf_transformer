...
public class NewMemberRunner implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        TableMapReduceUtil.initTableMapperJob(this.initScan(job), NewMemberMapper.class, StatsUserDimension.class, TimeOutputValue.class, job, false);
        ...
        if (job.waitForCompletion(true)) {
            ...
            //job运行成功
            this.calculateTotalMember(job.getConfiguration());
            return 0;
        } else {
            //运行失败
            return -1;
     ...
     * @param conf
     */
    private void calculateTotalMember(Configuration conf) {
        ...
        try {
            ...
            for (Map.Entry<String, Integer> entry : oldValueMap.entrySet()) {
                i = 0;
                pstmt.setInt(++i, Integer.valueOf(entry.getKey()));
                pstmt.setInt(++i, todayDimensionId);
                pstmt.setInt(++i, entry.getValue());
                pstmt.setInt(++i, entry.getValue());
                pstmt.execute();
            }
            ...
            for (Map.Entry<String, Integer> entry : oldValueMap.entrySet()) {
                ...
                pstmt.setInt(++i, Integer.valueOf(keys[0]));
                pstmt.setInt(++i, Integer.valueOf(keys[1]));
                pstmt.setInt(++i, todayDimensionId);
                pstmt.setInt(++i, entry.getValue());
                pstmt.setInt(++i, entry.getValue());
                pstmt.execute();
