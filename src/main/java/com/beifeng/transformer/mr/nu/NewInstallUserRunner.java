...
public class NewInstallUserRunner extends TransformerBaseRunner {
    ...
    public static void main(String[] args) {
        ...
        runner.setupRunner("new-install-user", NewInstallUserRunner.class, NewInstallUserMapper.class, NewInstallUserReducer.class, StatsUserDimension.class, TimeOutputValue.class, StatsUserDimension.class, MapWritableValue.class);
        try {
    ...
    protected void afterRunJob(Job job, Throwable error) throws IOException {
        try {
            if (error == null && job.isSuccessful()) {
                ...
                //job运行没有产生异常，但是运行失败
                throw new RuntimeException("job运行失败");
            }
        } catch (Throwable e) {
            if (error != null) {
                error = e;
